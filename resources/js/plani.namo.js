    ...
    */
	...
	if(cuManagerBoolean){
	    if(createType.indexOf('BASE') > -1){
	    	...
	    	managerProperties.width = '900';            // FileUploadManager 너비
	    	managerProperties.height = '320';          // FileUploadManager 높이
			// FileUploadManager 객체가 생성될 html div 태그 id
	    	managerProperties.containerId = (createType.split('&').length > 1 ? objectId.split('||')[0]:objectId);
	    	managerProperties.uploadUrl = filePath;             // 파일 업로드 처리 페이지 경로
	    	managerProperties.uploadButtonVisible = false;      // 업로드 버튼 Visible 설정
			managerProperties.uploadButtonDisplayStyle = 'none';	// 업로드 버튼 숨기기
	...
	if(cuManagerImgBoolean){
	    if(createType.indexOf('IMG') > -1){
	    	var managerImgProperties = new Object();
	    	managerImgProperties.width = '900';            // FileUploadManager 너비
	    	managerImgProperties.height = '320';          // FileUploadManager 높이
			// FileUploadManager 객체가 생성될 html div 태그 id
	    	managerImgProperties.containerId = (createType.split('&').length > 1 ? objectId.split('||')[1]:objectId);
	    	managerImgProperties.uploadUrl = filePath;             // 파일 업로드 처리 페이지 경로
	    	managerImgProperties.uploadButtonVisible = false;      // 업로드 버튼 Visible 설정
			managerImgProperties.uploadButtonDisplayStyle = 'none';	// 업로드 버튼 숨기기

	    	var monitorImgProperties = new Object();
	    	monitorImgProperties.monitorLayerClass = 'monitorLayer';      // FileUploadMonitor 창의 스타일입니다. (namocrossuploader.css 파일에 정의, 변경 가능)
	    	monitorImgProperties.monitorBgLayerClass = 'monitorBgLayer';  // FileUploadMonitor 창의 백그라운드 스타일입니다. (namocrossuploader.css 파일에 정의, 변경 가능)
	    	monitorImgProperties.closeMonitorCheckBoxChecked = true;      // 전송 완료 후 FileUploadMonitor 창 닫기 설정

		    cuManagerImages = namoCrossUploader.createUploader(
		        JSON.stringify(managerImgProperties),                          // FileUploadManager 프로퍼티를 JSON 문자열로 전달
		        JSON.stringify(monitorImgProperties),                          // FileUploadMonitor 프로퍼티를 JSON 문자열로 전달
		        JSON.stringify(window.namoCrossUploaderConfig.eventNames)); // 이벤트 이름을 JSON 문자열로 전달
...
var onExceptionCu = function () {
	...
	// 필요한 예외정보만 고객에서 보여주십시오.
	var exceptionInfo = "";
	if(createType.indexOf('BASE') > -1) exceptionInfo = cuManager.getLastExceptionInfo();
	if(createType.indexOf('IMG') > -1) exceptionInfo = cuManagerImages.getLastExceptionInfo();
...
var onCloseMonitorWindowCu = function () {
	...
	if(cuManager != null && cuManager.getUploadStatus() == "COMPLETION") {
		/**
