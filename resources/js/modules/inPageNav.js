...
export function inPageNav(el) {
    ...
    if ($pageTit.length > 1) {
        ...
        const $navListUl = $('.in-page-navigation-list ul');

        $pageTit.each(function (index) {
            const text = $(this).text();
            $(this).attr('id', `anchor-page-${index}`);
            $navListUl.append(
                `<li>
                    <a href="#anchor-page-${index}" data-anchor="anchor-page-${index}">${text}</a>
                </li>`
            );
        });
    }
    ...
    // 탐색 메뉴 클릭 - 스크롤 핸들러 외부에 등록해야 중복 바인딩 방지
    ...
    $win.on('load scroll', () => {
        ...
        if (scrollT > 110) {
            ...
            if (scrollT + $win.height() >= $(document).height()) {
                $navList.find('li:last a').addClass(AC)
                    .parent().siblings('li').find('a').removeClass(AC).addClass('up');
            }
        }
    });
}
